...
| WhatsApp Cloud API (Meta) | REQ-BP01-5, REQ-BP07-3, REQ-BP10-5, generally | Code architecture is fully wired (webhook, outbound text/interactive/template senders, conversational commerce engine — Master Rule Set Module 13, this session) — **CONFIGURED** in code, but **no real WhatsApp Business Account credentials exist in this environment**: `.env`/`.env.example` and every process environment checked this session carry only placeholder values (`MY_WHATSAPP_ACCESS_TOKEN`, etc.), confirmed by `isWhatsAppConfigured()` returning `false`. The earlier "Fully wired, live"/AVAILABLE claim in this row (pre-dating this session) conflated "the code is wired" with "credentials are live" — corrected here. | **REQUIRES_CREDENTIALS** (real `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_APP_SECRET`, `WHATSAPP_WEBHOOK_VERIFY_TOKEN` from Meta Business Manager); everything downstream of that (webhook processing, the conversation engine, the reservation-creation path) is already built and verified against a simulated, correctly-signed delivery — see Module 13. |
