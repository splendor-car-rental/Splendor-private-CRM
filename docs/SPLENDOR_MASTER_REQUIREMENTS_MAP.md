...
| REQ-BP03-1..5 (blocklist/watchlist + tiered blocking + unblock SoD) | **PARTIALLY_IMPLEMENTED** | `CustomerStatus` includes `'blocklisted'` (`src/types/index.ts:18`); a status-change notification fires on transition into it (`server.ts:909-916`, `dispatchNotificationEvent('customer_blocklisted', ...)`). This is a **manual status flag with a notification**, not the described silent-proactive-check-on-entry engine, not tiered (full vs. conditional) blocking, not a shared cross-referenced table matched by passport+country/Emirates-ID pairs, and no dedicated unblock-approval workflow was found beyond the app's generic Segregation-of-Duties primitive (`approvals.ts`) which is not wired to this specific transition. |
...
| REQ-BP08-1 (8 mandatory angle photos) | **PARTIALLY_IMPLEMENTED — different shape** | `HandoverInspection`/`ReturnInspection` capture `startMileage`/`endMileage`, `fuelLevelPercent`, `cleanliness`, and a `VehicleDamageMarker[]` list (`src/types/index.ts:474-523`) — no evidence of an 8-fixed-angle mandatory photo requirement or image-quality (blur/darkness) rejection gate. |
| REQ-BP08-2 (tap-exact-point 3D model) | **PARTIALLY_IMPLEMENTED — simpler shape** | `VehicleDamageMarker` (`src/types/index.ts:466-472`) records damage by a **fixed enum of vehicle parts** (`front_bumper`, `rear_bumper`, `hood`, ... 11 parts) + a severity enum + notes + an optional photo — not a free-form coordinate/pixel-position marker on an interactive 3D model. |
| REQ-BP08-3 (handover-vs-return side-by-side comparison, auto-flag new damage) | **UNKNOWN — data exists, comparison UI not confirmed** | Both `HandoverInspection.damages` and `ReturnInspection.newDamages` exist on `Contract` (`src/types/index.ts:552-553`), so a UI *could* diff them, but no dedicated side-by-side comparison screen was confirmed in this pass (would need a targeted UI trace — not performed here to preserve efficiency; flagged for follow-up). |
| REQ-BP08-4/5 (forced-flash/geo-time-stamped photos, blur/darkness rejection) | **MISSING** | No image-quality-check code, no forced-flash control, no geo/time-stamp-on-photo mechanism found. |
| REQ-BP09-1..5 (odometer accumulation, maintenance thresholds, auto-block, projected-mileage warning, auto-substitution) | **MISSING** | No maintenance-scheduling module exists in `src/server/` (28-module inventory in `docs/REPOSITORY_INVENTORY.md` has no maintenance/odometer-threshold module). Vehicle status supports a `'maintenance'` value used by the availability engine (`src/server/availability.ts:76`) as a hard status-block, but nothing computes *when* to enter that status from accumulated mileage, and no auto-substitution engine was found. |
...
| REQ-BP10-2 (mandatory 3-hour buffer between bookings) | **MISSING** | Read in full: `reserveVehicleSlot()`'s overlap check (`src/server/availability.ts:80-102`) is a direct date-range overlap test with **no buffer padding at all** — two bookings with a 1-minute gap would both be accepted. The specific "+3 hours" business rule described in the Blueprint is not present in this function or anywhere else searched. |
| REQ-BP10-4 (10-minute soft hold during checkout) | **MISSING** | No TTL-based temporary/soft lock distinct from the final atomic write was found; this concept applies to a public booking website flow, which itself was not found in this repository's scope (see §23). |
...
| REQ-BP11-1..5 (event calendar, dynamic pricing, quote lock, discount ceiling) | **MISSING** | Zero matches for `dynamicPric`, `surgePric`, `eventPricing`, `seasonalPric` anywhere in the codebase. No pricing-rule engine, no Dubai-events calendar, no quote-lock-with-reference-number mechanism, no discount-percentage ceiling enforcement was found (a per-role discount cap is plausible via `config/permissions.ts`'s role-rank model, but this was not confirmed to implement the specific "5% cap, escalate above" rule described). |
...
| REQ-BP12-4 (cryptographic hash-chaining, tamper detection on delete) | **MISSING — specific, confirmed gap** | `recordAudit()`'s implementation (`server.ts:277-283`) is a plain sequential append with **no hash of the previous entry stored or verified**. The current protection is "no code path can delete an entry" (a real, meaningful protection) but **not** the Blueprint's specific claim of automatic tamper detection if an entry is removed directly via the Admin SDK/Firestore console. |
