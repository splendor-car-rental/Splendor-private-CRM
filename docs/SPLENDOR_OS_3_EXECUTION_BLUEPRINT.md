...
main (trusted, production)
   └── stabilization/clean-recovery-20260903  [ONLY active integration branch, PR #56]
          ...
          ├── P0 done ✅ (split-brain closed for deposits/payments/plate assignment,
          │    debt-settlement idempotency, active-staff status enforcement,
          │    env/cold-start audit — see Verified Current State)
...
## P0 — SECURITY & DATA INTEGRITY (done, pending final CI confirmation on the last pushed SHA)
Every item below fixed and shipped this session, each its own gated, tested commit on `stabilization/clean-recovery-20260903`:

- Issue #36 full contract lifecycle (`draft → review → approved → signed → active → settlement_pending → completed`, physical return never itself closing the contract or recognizing LTV, `/close` the sole recognition event).
- `assignPlateAtomically` unification (server.ts now calls the one real atomic implementation instead of a legacy shadow path).
- **Dual-entrypoint split-brain, closed across every instance found**: `api/index.ts` (the real Vercel production entrypoint) intercepts a small explicit set of security/financial-critical routes with hardened, transactional, verified-actor implementations, then falls through to the full legacy Express app in `server.ts` for everything else. Several of those intercepted paths had a **second, older, less-safe implementation still live inside `server.ts` itself** — dead code in Vercel production (shadowed) but reachable via local dev and every existing test file (which calls `server.ts` directly via supertest, never through `api/index.ts`). Fixed for: plate assignment, `POST /api/deposits` (was `createSecurityDeposit()`, no journal, no idempotency), `POST /api/deposits/:id/(apply|refund)` (was a correct-but-non-journal-posting inline transaction), `POST /api/payments` (was `createConfirmedPayment()`, also let `corporate_credit` be recorded as a fake cash receipt — removed that option from the manual-payment UI). All four now delegate to the exact function `api/index.ts` uses.
- **Swallowed accounting error messages** (found while writing real test coverage for the above, since it had none before): `safeDepositAccounting.ts`'s business-rule errors (already-deducted, not-posted, duplicate journal...) were plain `Error` thrown inside a Firestore transaction, which `runDurableTransaction` silently replaces with a generic "Transaction failed." — losing the real reason in production since PR #9. Fixed by throwing a proper `PersistenceError` subclass. The same pattern is flagged for the other `safe*.ts` accounting modules as a queued follow-up task (not yet fixed — see spawn_task `task_2b745e0f`).
- **Debt settlement idempotency**: `addDebtSettlement` had no Idempotency-Key protection at all (unlike deposits/payments) — a double-click/retry could record the same customer payment twice. Fixed the same way, plus added the route to `apiFetch.ts`'s auto-key list.
- Debt offsetting (`balances.ts`) and supplier payouts (`supplierPayments.ts`, `safePayablePayment.ts`) audited — both already correctly guarded from a prior session's concurrency pass (status-transition idempotency, live-balance re-derivation inside the approval transaction); no further fix needed.
- **`profile.status === 'active'` enforcement, the most significant finding**: `server.ts`'s own `getRequesterRole()` — called by `requireRole()`, the authorization gate on essentially every route in the whole application except the small set `api/index.ts` separately hardens — returned a profile's role unconditionally, **never checking status at all**. A deactivated/suspended/terminated staff member whose Firebase Auth session hadn't expired yet retained full role-based access to everything: fleet, customers, contracts, deposits, payments, debts. Fixed by making `getRequesterRole()` itself enforce `status === 'active'` (one change, all 8 call sites covered), matching the canonical check `api/index.ts` already used. Regression test added proving a suspended-but-role-valid account is now rejected.
- Repository-wide `process.env.*`/cold-start audit across every serverless entrypoint (`api/index.ts`, `api/contract-extension-safe.ts`, `api/fleet-safe.ts`, `api/whatsapp-webhook.ts`): checked — none read `process.env` or do other work at module scope; every credential/config read is lazy, inside a per-request handler, and fails closed (503/`false`) rather than crashing cold start or falling back to a hardcoded secret. No fix needed.

Exit gate: zero known Critical/High defect, tsc clean, full test suite green (including Firestore emulator + concurrency tests), CodeQL clean, production build clean — met on every commit above; re-verify the live GitHub Actions run on the last pushed SHA before treating P0 as formally closed.
