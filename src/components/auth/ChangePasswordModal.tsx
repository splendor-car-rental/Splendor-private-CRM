...
export const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleClose = () => {
    setNewPassword(''); setConfirmPassword(''); setError(null);
    onClose();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // UX invariant: only a confirmed Firebase success closes the form.
      // Failures remain visible in-place so the operator never has to guess.
      handleClose();
      showToast(t('passwordChangedSuccess'), t('passwordChangedSuccess'), 'success');
    } catch (err: any) {
