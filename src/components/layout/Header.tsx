    ...
    <>
      <header className="h-16 px-4 sm:px-6 bg-zinc-950/90 border-b border-zinc-800/80 backdrop-blur-md flex items-center justify-between gap-3 sm:gap-4 sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          <div className="flex items-center bg-zinc-900/70 p-0.5 rounded-xl border border-zinc-800">
            <button
              onClick={() => setLanguage('ar')}
              className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${
                language === 'ar'
                  ? 'bg-[#D4AF37]/20 text-[#f5d97f] border border-[#D4AF37]/35 shadow-sm'
            ...
            {quickMenuOpen && (
              ...
              >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs text-zinc-200 hover:text-white hover:bg-zinc-900 text-start transition-all group"
                >
                  <Car className="w-4 h-4 text-emerald-400 group-hover:scale-105 transition-transform" />
                  <div>
                    <div className="font-medium">{language === 'ar' ? 'إضافة سيارة للأسطول' : 'Add Vehicle to Fleet'}</div>
                    <div className="text-[10px] text-zinc-500">{language === 'ar' ? 'سوبركارز وفحص التوفر' : 'Fleet inventory'}</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <div className="font-medium">{language === 'ar' ? 'تسجيل عميل VIP جديد' : 'Register VIP Customer'}</div>
                    <div className="text-[10px] text-zinc-500">{language === 'ar' ? 'ملف 360 ومنع التكرار' : 'Customer KYC profile'}</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <div className="font-medium">{language === 'ar' ? 'إصدار عقد إيجار لحظي' : 'Issue Instant Contract'}</div>
                    <div className="text-[10px] text-zinc-500">{language === 'ar' ? 'تحديث فوري للإيرادات' : 'Live rental contract'}</div>
                  </div>
                </button>
              </div>
            )}
