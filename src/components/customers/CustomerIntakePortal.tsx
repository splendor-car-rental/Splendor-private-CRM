...
const initialState: IntakeState = {
  mode: 'individual', fullName: '', email: '', phoneCountry: 'AE', phoneLocal: '', nationality: 'AE', address: '', city: 'Dubai', residenceCountry: 'AE',
  idType: 'emirates_id', idNumber: '', idIssueDate: '', idExpiryDate: '', idIssuedBy: 'AE', licenseType: 'uae', licenseNumber: '', licenseIssueDate: '', licenseExpiryDate: '', licenseCountry: 'AE',
  companyName: '', tradeLicenseNumber: '', tradeLicenseIssueDate: '', tradeLicenseExpiryDate: '', tradeLicenseIssuedBy: 'AE', companyOwnerName: '', responsibleManagerName: '', companyPhoneCountry: 'AE', companyPhoneLocal: '', companyAddress: '', companyTrn: '', companyEmail: ''
};
...
export const CustomerIntakePortal: React.FC = () => {
  ...
  const duplicateCheck = async () => {
    ...
    try {
      const result = await checkDuplicateCustomer(email, phoneValue, form.licenseNumber, form.idNumber);
        ...
        <form onSubmit={submit} className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {individual ? (
            <div className="space-y-6">
              <section className="rounded-2xl border border-sky-500/15 bg-zinc-900/45 p-4"><h3 className="mb-4 text-sm font-black text-sky-300">{language === 'ar' ? 'البيانات الأساسية' : 'Basic Information'}</h3><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <div><label className={labelClass}>{language === 'ar' ? 'الاسم الكامل *' : 'Full legal name *'}</label><input required value={form.fullName} onChange={e => set('fullName', e.target.value)} className={fieldClass} /></div>
                ...
                <div><label className={labelClass}>{language === 'ar' ? 'العنوان' : 'Address'}</label><input value={form.address} onChange={e => set('address', e.target.value)} className={fieldClass} /></div>
              ...
              <section className="rounded-2xl border border-sky-500/15 bg-zinc-900/45 p-4"><h3 className="mb-4 text-sm font-black text-sky-300">{language === 'ar' ? 'الهوية أو جواز السفر' : 'Identity / Passport'}</h3><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الإصدار *' : 'Issue date *'}</label><input required type="date" value={form.idIssueDate} onChange={e => set('idIssueDate', e.target.value)} className={fieldClass} /></div>
                <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الانتهاء *' : 'Expiry date *'}</label><input required type="date" value={form.idExpiryDate} onChange={e => set('idExpiryDate', e.target.value)} className={fieldClass} /></div>
              </div></section>

              <section className="rounded-2xl border border-sky-500/15 bg-zinc-900/45 p-4"><h3 className="mb-4 text-sm font-black text-sky-300">{language === 'ar' ? 'رخصة القيادة' : 'Driving Licence'}</h3><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <div><label className={labelClass}>{language === 'ar' ? 'نوع الرخصة' : 'Licence type'}</label><select value={form.licenseType} onChange={e => set('licenseType', e.target.value as IntakeState['licenseType'])} className={fieldClass}><option value="uae">{language === 'ar' ? 'رخصة إماراتية' : 'UAE licence'}</option><option value="international">{language === 'ar' ? 'رخصة دولية' : 'International licence'}</option><option value="gcc">{language === 'ar' ? 'رخصة مجلس التعاون الخليجي' : 'GCC licence'}</option></select></div>
                ...
                <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الإصدار *' : 'Issue date *'}</label><input required type="date" value={form.licenseIssueDate} onChange={e => set('licenseIssueDate', e.target.value)} className={fieldClass} /></div>
                <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الانتهاء *' : 'Expiry date *'}</label><input required type="date" value={form.licenseExpiryDate} onChange={e => set('licenseExpiryDate', e.target.value)} className={fieldClass} /></div>
              </div></section>
            </div>
          ) : (
            <section className="rounded-2xl border border-sky-500/15 bg-zinc-900/45 p-4"><h3 className="mb-4 text-sm font-black text-sky-300">{language === 'ar' ? 'بيانات الشركة' : 'Company Information'}</h3><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <div><label className={labelClass}>{language === 'ar' ? 'اسم الشركة *' : 'Company name *'}</label><input required value={form.companyName} onChange={e => set('companyName', e.target.value)} className={fieldClass} /></div>
              ...
              <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الإصدار *' : 'Issue date *'}</label><input required type="date" value={form.tradeLicenseIssueDate} onChange={e => set('tradeLicenseIssueDate', e.target.value)} className={fieldClass} /></div>
              <div><label className={labelClass}>{language === 'ar' ? 'تاريخ الانتهاء *' : 'Expiry date *'}</label><input required type="date" value={form.tradeLicenseExpiryDate} onChange={e => set('tradeLicenseExpiryDate', e.target.value)} className={fieldClass} /></div>
