...
function table(headers: string[], rows: Array<Record<string, unknown>>): string {
  const keys = Object.keys(rows[0] || {});
  return `<table><thead><tr>${headers.map(h => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr>${keys.map(k => `<td>${text(row[k]) || '—'}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}
...
function renderBody(input: CorporateDocumentInput, serial: string): string {
  ...
  switch (input.kind) {
    ...
    case 'credit_note':
    case 'debit_note':
      return `${heading}<h2>بيانات الإشعار</h2>${fieldsBlock({ 'اسم العميل': customer.name, 'الرقم الضريبي للعميل (TRN)': customer.trn, 'رقم الفاتورة الأصلية': f.originalInvoiceNumber, 'تاريخ الفاتورة الأصلية': f.originalInvoiceDate, 'رقم عقد الإيجار': f.contractNumber, 'رقم اللوحة': vehicle.plateNumber, 'سبب الإصدار': f.reason })}${rows.length ? table(['م', 'البيان', 'الكمية', 'سعر الوحدة', 'المبلغ قبل الضريبة', 'الضريبة %', 'قيمة الضريبة', 'الإجمالي شامل الضريبة'], rows) : ''}${summaryBox([['الإجمالي قبل الضريبة', f.subtotal], ['ضريبة القيمة المضافة (5%)', f.vat]], [input.kind === 'credit_note' ? ['إجمالي قيمة الإشعار الدائن', f.total] : ['إجمالي قيمة الإشعار المدين', f.total]])}${notesBlock(input.notes)}<div class="signature-grid"><div>توقيع العميل: ____________________</div><div>ختم وتوقيع سبلندر لتأجير السيارات: ____________________</div></div>`;
    case 'fines_notice':
