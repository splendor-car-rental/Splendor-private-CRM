...
declare module './index' {
  ...
  export interface Vehicle {
    chassisNumber?: string; engineNumber?: string; plateCategory?: string; registrationDate?: string;
    trafficFileNumber?: string; insuranceCompany?: string; insuranceType?: string; insurancePolicyNumber?: string;
    mortgagee?: string; manufacturingCountry?: string; seatingCapacity?: number; gpsTrackingCompany?: string;
    gpsCertificateExpiry?: string; dailyMileageAllowance?: number; monthlyMileageAllowance?: number; extraKmRate?: number;
  }
  export interface Contract { monthlyMileageAllowance?: number; currency?: string; exchangeRate?: number; }

  export interface CompanyBankAccount {
    id: string; bankName: string; bankNameAr?: string; accountName?: string; accountNameAr?: string;
    accountNumber?: string; iban?: string; swiftCode?: string; swiftBic?: string; currency?: string; branch?: string;
    openingBalance?: number; status?: 'active' | 'inactive' | string; isPrimary?: boolean; notes?: string;
  ...
  export interface SecurityDepositRecord {
    id: string; customerId: string; contractId?: string; reservationId?: string; amount: number; appliedAmount?: number;
    refundedAmount?: number; balance?: number; currentBalance?: number; totalDeductions?: number; deductions?: any[];
    paymentMethod?: string; status: DepositLifecycleStatus; holdReleaseDueDate?: string; holdReleaseEligibleDate?: string;
    appliedReason?: string; refundDate?: string; returnDate?: string; releasedAt?: string; releasedBy?: string;
    transactionRef?: string; auditHistory?: any[]; notes?: string; createdAt?: string; updatedAt?: string;
  }
  ...
  export interface TrafficFine {
    id: string; vehicleId?: string; vehiclePlate?: string; customerId?: string; contractId?: string; reservationId?: string;
    date: string; amount: number; status: FineStatus; source: FineSource; referenceNumber?: string; description?: string;
    evidenceUrl?: string; createdAt?: string; updatedAt?: string;
  }

  export interface TelematicsPing {
    id: string; vehicleId: string; timestamp: string; latitude: number; longitude: number; speed?: number; speedKmh?: number;
    heading?: number; mileage?: number; ignition?: boolean; metadata?: Record<string, unknown>;
  }
  export interface TelematicsAlert {
    id: string; vehicleId: string; contractId?: string; type: string; severity?: 'low' | 'medium' | 'high' | 'critical' | string;
    message?: string; timestamp: string; resolved?: boolean; resolvedAt?: string; resolvedBy?: string; metadata?: Record<string, unknown>;
  }
  export interface GeofenceZone {
    id: string; name: string; nameAr?: string; description?: string; latitude?: number; longitude?: number; radiusMeters?: number;
    coordinates?: Array<{ latitude: number; longitude: number }>; radius?: number; zoneType?: string; penaltyAmountAed?: number;
    active?: boolean; polygon?: Array<{ latitude: number; longitude: number }>; createdAt?: string; updatedAt?: string;
  }

  export interface SeasonalityRule {
    id: string; name: string; nameAr?: string; startDate?: string; endDate?: string; multiplier: number; active?: boolean;
    priority?: number; applicableCategories?: string[]; baseDailyRate?: number;
  }
  ...
  export interface YieldPricingQuoteResult {
    ...
    durationDays?: number; occupancyMultiplier?: number; seasonalityMultiplier?: number; breakdown?: Record<string, number>;
  }
