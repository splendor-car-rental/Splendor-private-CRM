...
describe('Tax Reconciliation runtime boundary', () => {
  ...
  it('captures immutable evidence only from authoritative accounting journals and source records', () => {
    expect(runtime).toContain("const JOURNAL_COLLECTION = 'accounting_journals'");
    expect(runtime).toContain("journal.status === 'posted'");
    expect(runtime).toContain("tx.get(firestore.collection('invoices'))");
    expect(runtime).toContain("tx.get(firestore.collection('payments'))");
    expect(runtime).toContain("tx.get(firestore.collection('supplier_invoices'))");
  ...
  it('uses Four-Eyes review to resolve managed posting gaps and exposes no filing/delete path', () => {
    ...
    expect(runtime).toContain("status: 'reviewed_clean'");
